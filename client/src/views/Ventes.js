...
export default function Ventes() {
	...
	const ventesState 	= useSelector(state => state.ventesState); 
	...
	useEffect(() => {
		if (!loadingState) {
			setContactsMenu({
				clients: contactsState.filter(x => x.type === "client").map(x => x.nom),
				commercials: contactsState.filter(x => x.type === "commercial").map(x => x.nom),
			});
	...
	const settings = {
		...
		afterChange(changes, source) {
			...
			changes.forEach(([row, prop, oldVal, newVal]) => {
				...
				if (["date", "commercial", "client"].includes(prop)) {
					if (newVal === null) {
				...
				if (prop === "ca") {
					...
					const reste = this.getDataAtRowProp(row, "reste");
					this.setDataAtRowProp(row, "reste", Number(reste) + dif);
				}
				...
				if (["espece", "cheque", "tpe", "virement"].includes(prop)) {
					...
					dispatch({ type: "caisse/add", payload: { 
						type: prop,
						montant: dif
					} });
				}
		...
		dataSchema: { date: getTodayDate(), commercial: "", nDeBon: "", nDeFacture: "", ca: 0, encaisse: 0, reste: 0, espece: 0, cheque: 0, tpe: 0, virement: 0, client: "" },
		columns: [
			{
				title: "Date",
				type: "date",
			...
			},
			{
				...
				type: "numeric",
				data: "reste", 
				// validator: resteValidator, invalidCellClassName: "myInvalidClass",
				numericFormat: { pattern: "0.00" },
				width: 100,
			},
			{
      ...
      // console.log(this.getCell(1, 1));
			// document.getElementsByTagName("td")[3].style.color = "red"
			// this.setDataAtCell(row, 0, "17/02/2021");
			// console.log(this);
			// this.getCell(1, 1).style.backgroundColor = "red";
			// this.setCellMeta(1, 1, "className", "myRow");
    // },
		// cell: [...Array(12).keys()].map(i => {
		//   return {row: 0, col: i, readOnly: true, backgroundColor: "red"}
		// }),
		// cell: [{row: 0, col: 5, readOnly: true, backgroundColor: "red"}],
		language: "fr-FR",
	};
